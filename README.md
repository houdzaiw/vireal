...
- Replicate Wan 2.7 异步生成闭环：任务幂等提交、签名 Webhook、PostgreSQL 转存队列、私有 R2 播放和 24 小时清理（默认关闭，不会误触发付费调用）。
- 文字/图片动态发布、动态流和详情。
...
原型页面位于 `../outputs/vireal-wan-video-h5/prototype/prototype_v1.0.html`。不要直接使用 `file://` 打开进行真实 API 验证；应通过 HTTP 提供页面，并启用 `backend=1` 测试模式：
...
```text
http://localhost:5173/prototype_v1.0.html?backend=1&api=http://localhost:8000
```

测试模式会使用设备登录、真实上传和带 App Token 的私有图片读取。后端通过同源代理读取图片，因此本地上传验证不依赖 R2 Bucket CORS。测试模式下仅显示“跳舞”，支持 5/10 秒，并固定为 720P、9:16、静音；当 Replicate 开关关闭时，点击生成会收到明确的 503，不会产生费用。
...
REPLICATE_POC_MAX_SUBMISSIONS=1
```
...
完整资源创建、环境变量、域名、无重定向 Webhook 和单次付费 PoC 步骤见 [`deployment-cloudflare-railway.md`](deployment-cloudflare-railway.md)。Railway 变量模板位于 [`deploy/railway-variables.example`](deploy/railway-variables.example)，公网验收脚本位于 [`scripts/verify-production.sh`](scripts/verify-production.sh)。
...
当前支付回调已支持 shared-secret 基础防护，但尚未实现 Apple/Google 官方生产级 JWS、Pub/Sub 或票据校验。首次 Replicate PoC 只开放单人跳舞；双人、15 秒、音频、服务端金币账本和 provider 轮询兜底均不在本轮范围内。`submission_unknown` 不会自动重新创建 prediction，以避免重复计费。
